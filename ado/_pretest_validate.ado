*! _pretest_validate v0.1.1 - Data Validation Module for pretest
*!
*! Purpose:
...
program define _pretest_validate, rclass
    ...
    syntax anything(name=outcome) [if] [in], TREATment(string) TIME(string)
    ...
    qui count if `touse' & `treatment' != 0 & `treatment' != 1 & !missing(`treatment')
    if r(N) > 0 {
        ...
        qui tab `treatment' if `touse' & `treatment' != 0 & `treatment' != 1 & !missing(`treatment'), matrow(badvals)
    ...
    // Outcome variable
    qui count if `touse' & missing(`outcome')
    if r(N) > 0 {
        local n_miss_y = r(N)
    ...
    // Treatment variable
    qui count if `touse' & missing(`treatment')
    if r(N) > 0 {
        local n_miss_d = r(N)
    ...
    // Time variable
    qui count if `touse' & missing(`time')
    if r(N) > 0 {
        local n_miss_t = r(N)
    ...
    // =========================================================================
    // Section 5: Staggered Adoption Detection
    // The pretest methodology requires block-adoption: all treated units must
    // share the same first treatment time t0 (Section 2.1 of M&H 2025).
    // If panel data shows time-varying treatment with multiple distinct first
    // treatment dates, the command cannot be validly applied as-is.
    // =========================================================================
    ...
    if _rc == 0 {
        ...
        if "`panelvar'" != "" & "`panelvar'" != "." {
            // Check if treatment varies within any panel unit over time
            tempvar _treat_sd
            qui bysort `panelvar': egen `_treat_sd' = sd(`treatment') if `touse'
            qui sum `_treat_sd'
            
            if r(max) > 0 {
                // Treatment is time-varying; collect first treatment time per unit
                tempvar _first_treat_time
                qui bysort `panelvar': egen `_first_treat_time' = ///
                    min(cond(`treatment' == 1, `time', .)) if `touse'
                
                // Count unique first treatment times among treated units
                qui levelsof `_first_treat_time' if `touse' & !missing(`_first_treat_time'), ///
                    local(_first_treat_times)
                local _n_first_times : word count `_first_treat_times'
                
                if `_n_first_times' > 1 {
                    di as error "Error: Staggered adoption detected."
                    di as error "  Treated units have `_n_first_times' distinct first" ///
                        " treatment times: `_first_treat_times'"
                    di as error "  pretest requires a block-adoption design where all" ///
                        " treated units"
                    di as error "  share the same treatment-onset time t0 (Section 2.1)."
                    di as error "  For staggered adoption, consider cohort-stacking" ///
                        " approaches before calling pretest."
                    exit 198
                }
            }
        }
    ...
    // =========================================================================
    // Section 6: Summary Statistics and Return Values
    // Report sample composition for treated (D=1) and control (D=0) groups.
    // =========================================================================
    
    // Count valid observations (complete cases)
    qui count if `touse' & !missing(`outcome') & !missing(`treatment') & !missing(`time')
    local n_valid = r(N)
    
    // Count by treatment status
    qui count if `touse' & `treatment' == 1 & !missing(`outcome') & !missing(`time')
    local n_treated = r(N)
    
    qui count if `touse' & `treatment' == 0 & !missing(`outcome') & !missing(`time')
    local n_control = r(N)
