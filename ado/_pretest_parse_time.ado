...
program define _pretest_parse_time, rclass
    ...
    qui levelsof `timevar' if `touse', local(time_levels)
    ...
    // --- Method 2: Infer from time-varying treatment in panel data ---
        ...
        if _rc == 0 {
            ...
            tempvar treat_sd
            qui bysort `panelvar': egen `treat_sd' = sd(`treatment') if `touse'
            
            qui sum `treat_sd' if `touse'
            if r(max) > 0 {
                // Treatment is time-varying; find first treated period
                qui levelsof `timevar' if `touse' & `treatment' == 1, local(treat_times)
