...
program define _pretest_graph
    ...
    if "`saving'" != "" {
        // Handle saving("filename", replace) subcase: when the user writes
        // saving("file.png", replace) the entire '"file.png", replace' lands
        // inside the captured string argument. Detect and split here so the
        // extension check and replace flag both work correctly.
        // Correct syntax: saving("file.png") replace  (replace as separate option)
        local _sf "`saving'"
        local _rf "`replace'"
        if regexm(`"`_sf'"', `"^"(.+)", *replace *$"') {
            local _sf = regexs(1)
            local _rf "replace"
        }
        else if regexm(`"`_sf'"', `"^(.+), *replace *$"') {
            local _sf = strtrim(regexs(1))
            local _rf "replace"
        }
        
        local ext = substr("`_sf'", -4, 4)
        
        if "`ext'" == ".gph" {
            graph save "`_sf'", `_rf'
        }
        else if "`ext'" == ".png" | "`ext'" == ".pdf" | "`ext'" == ".eps" {
            graph export "`_sf'", `_rf'
        }
        else {
            // Default to Stata native format
            graph save "`_sf'.gph", `_rf'
        }
        
        di as text "Graph saved to: `_sf'"
    }
