...
*!   This framework provides valid inference when testing violations of
...
*!   3. Compute asymptotic covariance matrix Sigma
*!   4. Calculate severity measure S_pre (Lp-norm of violations)
*!   5. Compute bias bound constant kappa (Proposition 1)
*!   6. Monte Carlo critical value f(alpha, Sigma) (Appendix D)
...
*!   _pretest_psi.mata         - Critical value computation (Appendix D)
 ...
 *   Step 2: Compute DID estimates delta_t (Section 2.1)
 *   Step 3: Compute violation estimates nu_t (Section 2.1)
 *   Step 4: Estimate asymptotic covariance Sigma (Assumption 1)
 *   Step 5: Calculate severity S_pre = ||nu||_p (Section 3.1)
 *   Step 6: Compute kappa constant (Proposition 1, Section 3.2)
 *   Step 7: Monte Carlo critical value f(alpha, Sigma) (Appendix D)
 *   Step 8: Execute pre-test phi = 1{S_pre > M} (Theorem 1, Section 4.2)
 *   Step 9: Construct CI: delta_bar +/- {kappa*S_pre + f/sqrt(n)} (Theorem 2)
 *
 * @param outcome_var   Outcome variable name in Stata dataset
 * @param treatment_var Treatment indicator variable (D in {0,1}, binary)
 * @param time_var      Time period variable (mapped to indices 1,...,T)
 * @param cluster_var   Cluster variable for robust SEs ("" if none)
 *
 * @note All parameters are communicated via Stata scalars with __pretest_ prefix
 * @note Results are stored in Stata scalars/matrices for retrieval by ado file
 *
 * Input Scalars:
 *   T, t0, T_pre, T_post   - Time structure parameters
 *   p, alpha, threshold    - Model specification (p-norm, significance, M)
 *   sims, seed, is_panel   - Simulation and data type parameters
 *   mode_overall           - 1=overall violations, 0=iterative violations
 *
 * Output Scalars:
 *   delta_bar  - Average DID estimate: delta_bar = (1/T_post) * sum delta_t
 *   S_pre      - Pre-treatment severity: S_pre = ((1/n) sum |nu_t|^p)^{1/p}
 *   kappa      - Bias bound constant from Proposition 1
 *   f_alpha    - Critical value f(alpha, Sigma) from Monte Carlo
 *   phi        - Pre-test result: 0=PASS (extrapolation valid), 1=FAIL
 *   ci_lower/upper - Conditionally valid CI bounds (only if phi=0)
 *
 * @see Mikhaeil & Harshaw (2025), Sections 2-5
 */
                           ...
                           string scalar cluster_var)
{
    ...
    // ========================================
    // Reference: Section 2.1, Equation for delta_t
    //
    // The DID estimand for period t >= t0:
    //   delta_t = E[Y_t - Y_{t0} | D=1] - E[Y_t - Y_{t0} | D=0]
    //
    // Sample estimator:
    //   delta_hat_t = (Y_bar_{t,1} - Y_bar_{t0,1}) - (Y_bar_{t,0} - Y_bar_{t0,0})
    //
    // where Y_bar_{t,d} denotes sample mean for group D=d at time t
    ...
    // ========================================
    // Step 4: Violation Estimation
    // ========================================
    // Reference: Section 2.1, Equations for nu_t and nubar_t
    //
    // Iterative violations (successive period differences):
    //   nu_t = E[Y^{(0)}_t - Y^{(0)}_{t-1} | D=1] - E[Y^{(0)}_t - Y^{(0)}_{t-1} | D=0]
    //   Sample: nu_hat_t = (Y_bar_{t,1} - Y_bar_{t-1,1}) - (Y_bar_{t,0} - Y_bar_{t-1,0})
    //
    // Overall violations (cumulative sum, Appendix D):
    //   nubar_t = sum_{s=2}^t nu_s
    //
    // The two representations are equivalent via: nubar_t = sum nu_s, nu_t = nubar_t - nubar_{t-1}
    ...
    // ========================================
    // Step 5: Asymptotic Covariance Estimation
    // ========================================
    // Reference: Section 2.2-2.3, Assumptions 1-2
    //
    ...
    // ========================================
    // Step 6: Severity Measurement
    // ========================================
    // Reference: Section 3.1, Definition of severity
    //
    // The severity of pre-treatment parallel trends violations:
    //   S_pre = ((1/(T_pre-1)) * sum_{t=2}^{t0-1} |nu_t|^p)^{1/p}
    //
    // Special cases:
    //   p=1:   Mean absolute violation
    //   p=2:   Root mean squared violation (default)
    //   p=inf: Maximum absolute violation
    real scalar S_pre
