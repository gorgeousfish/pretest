...
*!   Pre-test indicator (Section 4.2):
*!     phi = 1{S_pre > M}
*!
*! Reference:
...
real scalar _pretest_severity(real colvector nu_vec, | real scalar p)
{
    ...
    // 
    // Solution: Use scaling trick to avoid underflow.
    // Let max_abs = max(|nu|), then:
    //   S_pre = max_abs * (mean((|nu|/max_abs)^p))^(1/p)
    // 
    // Since |nu|/max_abs <= 1 with maximum = 1, scaled_values^p won't underflow.
    
    real colvector abs_nu
    real scalar max_abs, mean_scaled_p
    ...
    // For very large p (approaching L∞), use max directly to avoid any issues
    if (p >= 500) {
        // When p is very large, S_pre ≈ max(|nu|) = L∞ norm
        // Use scaled computation for numerical stability
        real colvector scaled
        scaled = abs_nu :/ max_abs  // All values in (0, 1], max = 1
        
        // Compute mean(scaled^p)^(1/p) * max_abs
        // For very large p, this approaches max_abs * 1 = max_abs
        mean_scaled_p = mean(scaled:^p)
        
        // Check for underflow in mean_scaled_p
        if (mean_scaled_p == 0 | mean_scaled_p < 1e-300) {
            // Underflow occurred even with scaling; fall back to L∞
            return(max_abs)
        }
        
        return(max_abs * mean_scaled_p^(1/p))
    }
    
    // For moderate p (3 to 499), use standard computation
    // This range is safe from underflow for typical violation magnitudes
    sum_abs_p = sum(abs_nu:^p)
    n = rows(nu_vec)
    
    // Check for underflow
    if (sum_abs_p == 0 | sum_abs_p < 1e-300) {
        // Underflow detected; use scaling method
        real colvector scaled2
        scaled2 = abs_nu :/ max_abs
        mean_scaled_p = mean(scaled2:^p)
        if (mean_scaled_p == 0 | mean_scaled_p < 1e-300) {
            return(max_abs)  // Fall back to L∞
        }
        return(max_abs * mean_scaled_p^(1/p))
    }
    
    return((sum_abs_p / n)^(1/p))
}
 ...
 *
 * @return Number of post-treatment periods: T_post = T - t0
 */
