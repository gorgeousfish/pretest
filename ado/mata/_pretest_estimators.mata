...
*! Difference-in-Differences Estimator Functions
*!
*! Implements DID estimators for the conditional extrapolation pre-test
*! framework following the potential outcomes approach.
*!
*! Functions:
*!   _pretest_group_mean()  - Period-group sample mean Y_bar_{t,d}
*!   _pretest_did_t()       - Single-period DID estimator delta_t
*!   _pretest_did_vector()  - DID vector (delta_{t0}, ..., delta_T)
*!   _pretest_did_avg()     - Average DID estimator delta_bar
*!
*! Key formulas (Paper Section 2.1):
 ...
 *
 * Implements the DID estimator for time period t (Section 2.1, Equation 171):
 *
 ...
 * @param D    Treatment indicator (n x 1), values in {0, 1}
 * @param time Time period index (n x 1 column vector)
 * @param t    Target period (t >= t0 for post-treatment)
 * @param t0   Treatment time (reference period)
 *
 * @return DID estimate delta_hat_t, or missing (.) if any group mean unavailable
 *
 * @note When t = t0, delta_{t0} = 0 by construction
 *
 * @see Mikhaeil & Harshaw (2025), Section 2.1, Equation (171)
 */
