                           ...
                           string scalar touse_var)
{
    ...
    // BUG-002: Load cluster_vec here (alongside Y/D/time) so it can be
    // filtered by the same valid_rows mask in Step 2.5
    if (cluster_var != "") {
        cluster_vec = st_data(., cluster_var)
    }
    
    // BUG-001: Load touse indicator to respect [if] [in] qualifiers
    if (touse_var != "") {
        touse_vec = st_data(., touse_var)
    }
    else {
        touse_vec = J(rows(Y), 1, 1)
    }
    ...
    // ========================================
    // Step 2.5: Filter observations (touse + missing value filter)
    // ========================================
    // BUG-001: Apply touse filter to respect [if] [in] qualifiers
    // BUG-002: Apply same filter to cluster_vec to keep alignment
    ...
    n_total = rows(Y)
    // Combine touse filter with missing-value filter
    valid_rows = touse_vec :& !rowmissing(Y) :& !rowmissing(D) :& !rowmissing(time_mapped)
    n_valid = sum(valid_rows)
    
    // Filter all data vectors to valid observations only
    if (n_valid < n_total) {
        Y = select(Y, valid_rows)
        D = select(D, valid_rows)
        time_mapped = select(time_mapped, valid_rows)
        // BUG-002: keep cluster_vec aligned with Y/D/time
        if (cluster_var != "") {
            cluster_vec = select(cluster_vec, valid_rows)
        }
    }
    ...
    // BUG-005: expose nubar in overall mode (iterative nu_vec otherwise)
    if (overall_flag == 1) {
        st_matrix("__pretest_nu", nubar_vec)
    }
    else {
    ...
    // BUG-002: cluster_vec is already loaded and filtered in Step 2/2.5
    if (cluster_var != "") {
    ...
    if (overall_flag == 1) {
        ...
        // BUG-005: expose overall-frame Sigma when in overall mode
        st_matrix("__pretest_Sigma", Sigma_hat_overall)
    }
    else {
        st_matrix("__pretest_Sigma", Sigma_hat)
    }
    
    // Extract Sigma_nu for violation component (always from iterative Sigma_hat)
    ...
    // BUG-005: use nubar_vec in overall mode so theta is in overall-frame coordinates
    if (overall_flag == 1) {
        theta_vec = nubar_vec \ delta_vec
    }
    else {
