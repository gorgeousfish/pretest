...
*!   critical values f(alpha, Sigma) in the conditional extrapolation
*!   pre-test framework for difference-in-differences designs.
*!
*! Key Quantities:
*!   Psi function (iterative mode, Section 5.1):
*!     psi(x) = |mean(x_post)| + kappa * S(x_pre, p)
*!   Psi function (overall mode, Appendix C):
*!     psi^Delta(x) = |mean(x_post)| + S(x_pre, p)
*!   Critical value (Appendix D):
                         ...
                         real scalar kappa)
{
    ...
    // Use LOG-SPACE computation to avoid numerical underflow for large p
    if (p >= 1e10) {
        // p = ∞: use max(|x_pre|)
        term2 = kappa * max(abs(x_pre))
    } else {
        // General p-norm with log-space numerical stability
        ...
        if (max_abs == 0 | max_abs < 1e-300) {
            ...
            // Handle zeros in abs_x
            abs_x = abs_x :+ (abs_x :== 0) * 1e-300
            
            log_max_abs = log(max_abs)
            log_abs_x = log(abs_x)
            log_ratios = log_abs_x :- log_max_abs  // All <= 0
            exp_terms = exp(p :* log_ratios)       // All in (0, 1]
                               ...
                               real scalar T_post, real scalar p)
{
    ...
    if (p >= 1e10) {
        ...
        if (max_abs2 == 0 | max_abs2 < 1e-300) {
            ...
            // Log-space computation for numerical stability
            // Handle zeros in abs_x2
            abs_x2 = abs_x2 :+ (abs_x2 :== 0) * 1e-300
            
            log_max_abs2 = log(max_abs2)
            log_abs_x2 = log(abs_x2)
            log_ratios2 = log_abs_x2 :- log_max_abs2  // All <= 0
            exp_terms2 = exp(p :* log_ratios2)        // All in (0, 1]
 ...
 *
 * where Z ~ N(0, Sigma) represents the asymptotic distribution of
 * sqrt(n) * (theta_hat - theta).
 *
 * Monte Carlo Algorithm:
 *   1. Regularize: Sigma_reg = Sigma + epsilon*I (numerical stability)
 *   2. Cholesky decomposition: L such that Sigma_reg = LL'
 *   3. For s = 1,...,S:
 *      a. Draw W^{(s)} ~ N(0, I_dim)
 *      b. Compute Z^{(s)} = L * W^{(s)} ~ N(0, Sigma_reg)
 *      c. Evaluate psi^{(s)} = psi(Z^{(s)})
 *   4. Return (1-alpha) quantile of {psi^{(1)}, ..., psi^{(S)}}
 *
 * @param Sigma_hat    Estimated asymptotic covariance matrix (T-1) x (T-1)
 * @param alpha        Significance level in (0, 1), typically 0.05
 * @param S            Number of Monte Carlo draws (default: 5000)
 * @param T_pre        Number of pre-treatment periods t0 - 1
 * @param T_post       Number of post-treatment periods T - t0 + 1
 * @param p            Norm index for severity (p >= 1 or inf)
 * @param kappa        Bias bound constant from Proposition 1
 * @param overall_mode 0 = iterative mode, 1 = overall mode
 * @param seed         Random seed for reproducibility
 *
 * @return Critical value f(alpha, Sigma_hat) for CI construction
 *
 * @note Cholesky ensures Z = LW with W ~ N(0,I) yields Z ~ N(0, Sigma)
 * @note Regularization epsilon = 1e-10 handles near-singular matrices
 * @note Uses ceiling index for conservative (anti-conservative in coverage) CV
 *
 * @see Mikhaeil & Harshaw (2025), Section 5.1, Appendix D
 */
