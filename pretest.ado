...
*!
*! Description:
*!   Implements a conditional extrapolation pre-test workflow for DID designs.
*!   This package provides:
*!   (1) Pre-test diagnostics for a user-chosen severity threshold M;
*!   (2) Pass-case interval endpoints centered on delta_bar when
*!       pretest_pass = 1 under the conditional extrapolation assumption.
*!
*! Syntax:
...
*!   alpha(#)         - Nominal significance level alpha in (0,1); if omitted,
*!                      derived from level() / current c(level)
*!   level(#)         - Nominal confidence level; if alpha() is omitted,
*!                      defaults to the current Stata confidence level
...
*!
*! Stored results:
*!   e(b)            - ATT-labeled coefficient vector storing delta_bar
*!   e(V)            - ATT-labeled variance slot for delta_bar
*!   e(T)            - Total number of time periods T
*!   e(t0)           - Treatment time t0 on the internal consecutive index
...
*!   e(phi)          - Decision code (0=pass, 1=threshold fail, .=invalid)
*!   e(kappa)        - Bias-bound scalar; overall mode fixes kappa at 1
*!   e(f_alpha)      - Critical value used in interval construction
*!   e(delta_bar)    - Average DID estimate delta_bar
*!   e(ci_lower)     - Lower pass-case interval endpoint when pretest_pass = 1
*!   e(ci_upper)     - Upper pass-case interval endpoint when pretest_pass = 1
*!   e(pretest_pass) - Pass indicator (1=pass, 0=non-pass)
...
program define pretest, eclass
    ...
    // =========================================================================
    if `treat_time' == -999 {
        _pretest_parse_time `time' if `touse', treatment(`treatment') is_panel(`is_panel')
    }
    else {
        _pretest_parse_time `time' if `touse', treatment(`treatment') ///
            treat_time(`treat_time') is_panel(`is_panel')
    }
    
    local T = r(T)
    local t0 = r(t0)
    local t0_orig = r(t0_orig)
    local T_pre = r(T_pre)
    ...
    mata: _pretest_main_compute("`outcome'", "`treatment'", "`time'", "`cluster'", "`touse'")
    ...
    // Build b vector (1 x 1): stores delta_bar (ATT-labeled slot for post-estimation compatibility)
    // Note: e(b)[1,1] = delta_bar, NOT the ATT level; see e(delta_bar) and e(ATT) alias
    matrix `b_mat' = J(1, 1, `delta_bar_result')
    matrix colnames `b_mat' = ATT
    matrix rownames `b_mat' = y1
    
    // Build V matrix (1 x 1): Var(delta_bar) stored in ATT-labeled slot
    ...
    ereturn scalar t0_orig = `t0_orig'
    ...
    // --- DID summary statistic delta_bar (ATT-labeled alias for post-estimation tools) ---
    // e(delta_bar) is the primary object; e(ATT) is an alias stored for compatibility
    // Neither is the ATT level; bias is bounded by kappa * S_pre under Assumption 3
    ereturn scalar delta_bar = `delta_bar_result' // delta_bar (DID summary statistic)
    ereturn scalar ATT = `delta_bar_result'       // ATT (alias for delta_bar; NOT the ATT level)
